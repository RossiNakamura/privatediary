...
class OnlyYouMixin(UserPassesTestMixin):
    ...
    def test_func(self):
        # URLに埋め込まれた主キーから日記データを１件取得、取得できなかった場合は４０４エラー
...
class DiaryListView(LoginRequiredMixin, generic.ListView):
    ...
    def get_queryset(self):
        diarys = Diary.objects.filter(user=self.request.user).order_by('-created_at')
        return diarys
...
class DiaryCreateView(LoginRequiredMixin, generic.CreateView):
    ...
    def form_invalid(self, form):
        messages.error(self.request, '日記の作成が失敗しました')
...
class DiaryUpdateView(LoginRequiredMixin, OnlyYouMixin, generic.UpdateView):
    ...
    def form_invalid(self, form):
        messages.error(self.request, '日記の更新が失敗しました')
        return super().form_invalid(form)
